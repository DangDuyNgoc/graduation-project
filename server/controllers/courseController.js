    ...
    const coursesWithMaterials = await Promise.all(
      courses.map(async (course) => {
        try {
          ...
          // check enrollment
          courseObj.isEnrolled = course.studentIds?.map((s) => s.toString()).includes(studentId);
  ...
  try {
    ...
    if (course.thumbnail?.public_id) {
      try {
        ...
        console.error("Error deleting Cloudinary thumbnail:", err);
      }
    }
...
export const deleteAllCourseController = async (req, res) => {
  try {
    ...
    // delete thumbnail from cloudinary
    const allThumbnailPublicIds = course.map((c) => c.thumbnail.public_id).filter(Boolean);

    if (allThumbnailPublicIds.length > 0) {
      try {
        await Promise.all(
          allThumbnailPublicIds.map(pid => cloudinary.v2.uploader.destroy(pid))
        )
      } catch (error) {
        console.error("Error deleting Cloudinary thumbnails:", err);
      }
    }
